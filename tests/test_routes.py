...
@pytest.mark.parametrize(
        "route, called_with, dbserv_sideeffect, status_code, response_text",  [
            ("/api/vessels", "vessels", CosmosResourceNotFoundError, 404, "document with id my_id not found in vessels"),
            ("/api/analyses", "analyses", CosmosResourceNotFoundError, 404, "document with id my_id not found in analyses"),
            ("/api/vessels", "vessels", CosmosHttpResponseError, 409, "error response from database"),
            ("/api/analyses", "analyses", CosmosHttpResponseError, 409, "error response from database"),
            ])
def test_get_vessel_not_in_db(app, route, called_with, dbserv_sideeffect, status_code, response_text):
    client = TestClient(app['app'])
#    db_response = {'foo': 'bar'}
...
def test_get_vessel(app, route, called_with):
    ...
    app['db_serv'].get_one_document_by_id.assert_called_once_with(called_with, "my_vessel_id")
    assert response.status_code == 200
    assert response.json() == db_response
...
@pytest.mark.parametrize(
        "route, called_with, obj_to_post",  [
            ...
            ("/api/analyses", "analyses", {'metadata':  {
                ...
                'general_results':{
                    ...
                    'm_extreme_nondrilling': 5},'all_seastate_results': [] }),
...
#    print(app['db_serv'].post_one_document.call_args_list)
    app['db_serv'].post_one_document.assert_called_once_with(called_with, {**obj_to_post, 'id':None})
    assert response.status_code == 200
    assert response.json() == db_response
...
def test_delete_analysis_id_not_in_db(app):
    ...
    app['db_serv'].delete_one_document_by_id.assert_called_once_with("analyses", "my_id")
...
def test_delete_vessel_(app):
    app['db_serv'].delete_one_document_by_id.side_effect = [{'title':'response_from_delete'}]
    app['db_serv'].get_analysis_id_by_vesselid.return_value = []
    print(app['db_serv'])
    client = TestClient(app['app'])
    response = client.delete("/api/vessels/my_id")
...
def test_delete_vessel_vessel_refered_to_in_analyses(app):
    app['db_serv'].delete_one_document_by_id.side_effect = [{'title':'response_from_delete'}]
    app['db_serv'].get_analysis_id_by_vesselid.return_value = [{'id':'my_id_1'}, {'id':'my_id_2'}]
    client = TestClient(app['app'])
    response = client.delete("/api/vessels/my_id")
    assert response.status_code == 409
    assert response.json() == {
...
def test_patch_vessel(app, patch_list, dbserv_response, expected_status_code, expected_json):
