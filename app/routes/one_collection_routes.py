...
def get_router_one_collection(
    ...
    if "get_by_id" in add_route_list:
        ...
        def get_one(id: str):
            try:
                ...
                return Response(
                    status_code=409, content=f"error response from database"
                )
    ...
    if "delete" in add_route_list:
        ...
        def delete(id: str):
            try:
                return_data = db_serv.delete_one_document_by_id(collection_name, id)
                return return_data
            except CosmosResourceNotFoundError:
                return Response(
                    status_code=404,
        ...
        def patch(id: str, updates: list[json_patch_modify]):
            ...
            try:
                return db_serv.patch_one_document(
                    collection_name=collection_name, document_id=id, updates=updates
                )
            except CosmosResourceNotFoundError:
                return Response(
                    status_code=404,
                    content=json.dumps(
                        {
                            "error_message": f"document with id {id} not found in {collection_name}"
                        }
                    ),
                )
            except CosmosHttpResponseError:
                return Response(
                    status_code=409,
                    content=json.dumps(
                        {"error_message": f"error response from database"}
                    ),
                )
            except Exception as e:
                raise e
